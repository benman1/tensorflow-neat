...
def add_connection(connections, genotype):
    enabled_innovations = [k for k in list(genotype.keys()) if genotype[k]]
    ...
    assert(len(list(genotype.keys())) <= len(connections))
...
def add_node(connections, genotype, debug=False):
    ...
    if not any(from_node == c[1] and new_node == c[2] for c in connections):
        ...
        ind = [c[0] for c in connections if c[1]
               == from_node and c[2] == new_node]
        genotype[ind[0]] = True
    ...
    if not any(new_node == c[1] and to_node == c[2] for c in connections):
        ...
        ind = [c[0]
               for c in connections if new_node == c[1] and to_node == c[2]]
        genotype[ind[0]] = True
...
def start_neuroevolution(x, y, x_test, y_test):
    ...
    connections = [(0, INPUT0, OUTPUT0), (1, INPUT1, OUTPUT0),
                   (2, INPUT0, OUTPUT1), (3, INPUT1, OUTPUT1)]
    ...
    for its in range(0, 100):
        ...
        fitnesses_sorted_indices = [
            i[0] for i in reversed(
                sorted(
                    enumerate(fitnesses),
                    key=lambda x: x[1]))]
        ...
        for ra in range(0, len(fitnesses_sorted_indices)):
            print(fitnesses[fitnesses_sorted_indices[ra]],
                  genotypes[fitnesses_sorted_indices[ra]])
        ...
        for i in range(0, m):
            print("adding:",
                  fitnesses[fitnesses_sorted_indices[i]],
                  genotypes[fitnesses_sorted_indices[i]])
