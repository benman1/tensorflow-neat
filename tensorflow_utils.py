...
def add_node(inputs, name="stdname"):
    with tf.name_scope(name):
        init_vals = tf.truncated_normal(
            [len(inputs), 1], stddev=1. / math.sqrt(2))
...
def build_and_test(connections, genotype, x, y, x_test, y_test, run_id="1"):
    ...
    # for all the same to's from connections collect from's (where genotype
    # exists and says enabled)
    ...
    connections_sorted = sorted(
        exisiting_connections,
        key=lambda connections: connections[2])

    # merge the same nodes
    connections_merged = [
        [connections_sorted[0][2], [connections_sorted[0][1]]]]
    for i in range(1, len(connections_sorted)):
        ...
        if connections_sorted[i][2] == connections_merged[-1][0]:
            ...
            connections_merged.append(
                [connections_sorted[i][2], [connections_sorted[i][1]]])
    ...
    num_nodes = tf.constant(len(list(tf_nodes_dict.keys())))
    ...
    with tf.name_scope("softmax_output"):
        ...
        output_final_pre = tf.transpose(
            tf.squeeze(tf.stack([output_0, output_1])))

        W_o1 = tf.Variable(tf.truncated_normal(
            [2, 2], stddev=1. / math.sqrt(2)))
        b_o1 = tf.Variable(tf.zeros([2]))
        output_final = tf.nn.softmax(
            tf.matmul(
                output_final_pre,
                W_o1) + b_o1,
            name="output_softmax")
    ...
    acc_test = sess.run([accuracy], feed_dict={x0: np.expand_dims(
        x_test[:, 0], 1), x1: np.expand_dims(x_test[:, 1], 1), y_: y_test})
    sess.close()
